...
Production must set `DATABASE_URL`, `ANTHROPIC_API_KEY`, restricted `ALLOWED_ORIGINS`, and non-default PostgreSQL credentials. SQLite remains available only as a unit-test/local compatibility path.
