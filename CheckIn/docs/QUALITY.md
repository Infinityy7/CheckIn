...
Run the complete local gate from `CheckIn/`:
...
a dummy Anthropic key and a disposable SQLite database for tests. Browser supplier
...
The gateway configuration `gateway/litellm.config.yaml` is version-controlled
with no secrets — values are substituted from the environment — and is covered
by the secret scan.
...
## LLM logging and PII posture

`character.md` and profiles are untrusted personal data, so full prompt/response
capture is PII. The app never logs prompt or profile text; success and failure
log lines carry a stable 16-hex `prompt_sha` for correlation only.
`LLM_LOG_PROMPTS` (default false) governs whether the optional LLM gateway may
capture message bodies, and `gateway/litellm.config.yaml` ships
`turn_off_message_logging: true` — keep the two in lockstep.
`LLM_LOG_RETENTION_DAYS` (default 30) documents gateway log retention. Refusal
explanations are never logged or returned. Keys and credentials never appear in
logs.
