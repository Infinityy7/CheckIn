...
Requirements: Python 3.11+, Node.js 20+, PostgreSQL 17 (or Docker), and an Anthropic API key. Duffel credentials are optional and only required for supplier-backed hotel and flight inventory.
...
Add `ANTHROPIC_API_KEY` to `.env`, then run:
...
```bash
.venv/bin/python scripts/migrate_sqlite_to_postgres.py --sqlite-path db/travelbuddy.db
```
