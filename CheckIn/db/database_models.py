...
class LLMResearchCache(Base):
    __tablename__ = "llm_research_cache"
    __table_args__ = (
        Index("ix_llm_research_cache_key_created", "exact_key", "created_at"),
    )

    cache_id: Mapped[str] = mapped_column(UUID_TYPE, primary_key=True, default=new_uuid)
    exact_key: Mapped[str] = mapped_column(String(64), nullable=False)
    taste_vector: Mapped[list[float]] = mapped_column(JSON_TYPE, nullable=False)
    request_facts: Mapped[dict[str, Any]] = mapped_column(JSON_TYPE, nullable=False)
    payload: Mapped[list[dict[str, Any]]] = mapped_column(JSON_TYPE, nullable=False)
