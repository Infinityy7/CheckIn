...
export function ItineraryView({ itinerary, preferences, postTrip, onBack, onRate }: { itinerary: Itinerary; preferences: TripPreferences; postTrip?: PostTripState; onBack: () => void; onRate: (rating: 1 | 2 | 3 | 4 | 5) => Promise<void> }) {
  ...
  return <main className="itinerary page-stage">
    ...
    <div className="itinerary-toolbar"><Button variant="quiet" onClick={onBack}>← Back to shortlist</Button><div><Button variant="secondary" onClick={() => navigator.clipboard?.writeText(window.location.href)}><Share2 /> Share</Button><Button onClick={() => window.print()}><Download /> Print / save PDF</Button></div></div>
    <section className="itinerary-body">
      <aside className="itinerary-index"><span className="eyebrow">Trip index</span>{plan.days.map((day) => <a href={`#day-${day.day_number}`} key={day.day_number}><b>0{day.day_number}</b><span>{day.theme}</span></a>)}<div className="pacing-note"><Sparkles /><p><strong>Tavi’s pacing note</strong>{plan.days.some((d) => d.items.length > 5) ? 'One day is fairly full. Consider moving the final stop into free time.' : 'This route keeps a comfortable rhythm with room for detours.'}</p></div></aside>
      <div className="day-plans">{plan.days.map((day, dayIndex) => <article className="day-plan" id={`day-${day.day_number}`} key={day.day_number}>
        <header><span>DAY {String(day.day_number).padStart(2, '0')}</span><div><small>{day.date}</small><h2>{day.theme}</h2></div></header>
        <div className="timeline">{day.items.map((item: ItineraryItem, itemIndex) => { const Icon = iconFor(item.category); return <div className="timeline-item" key={`${item.time_slot}-${item.title}`}>
          <div className="timeline-item__rail"><span><Icon /></span><i /></div>
          <div className="timeline-item__time"><Clock3 /> {item.time_slot}</div>
          <div className="timeline-item__card"><div><span className="category-label">{item.category}</span><h3>{item.title}</h3><p>{item.description}</p><div className="timeline-item__meta"><span><MapPin /> {item.location}</span><b>{item.cost_estimate}</b></div>{item.tip && <blockquote>Local note · {item.tip}</blockquote>}</div><div className="reorder"><button aria-label="Move earlier" onClick={() => move(dayIndex, itemIndex, -1)} disabled={itemIndex === 0}><ArrowUp /></button><button aria-label="Move later" onClick={() => move(dayIndex, itemIndex, 1)} disabled={itemIndex === day.items.length - 1}><ArrowDown /></button></div></div>
        </div>})}</div>
      </article>)}</div>
    </section>
