...
function Source({ data }: { data: FlightAvailability }) {
  const label = data.isLive && data.sourceMode === 'live' ? 'Live flight offers' : data.sourceMode === 'test' ? 'Test flight offers' : data.sourceMode === 'demo' ? 'Demo flight offers' : 'Provider unavailable'
  return <span className={`inventory-source inventory-source--${data.sourceMode}`}><i aria-hidden="true" /> {label} · {data.source}</span>
}
...
function Offer({ tripId, recommendationId, offer, onCartChange, onAdded }: {
  ...
  return <article className={`flight-offer flight-offer--${offer.availabilityStatus}`}>
    ...
    <ExpiryCountdown expiresAt={offer.holdExpiresAt ?? offer.quoteExpiresAt} kind={offer.holdExpiresAt ? 'hold' : 'quote'} />
...
export function FlightOffers({ tripId, recommendation, onCartChange, onAdded }: {
  ...
  return <section className="flight-inventory">
    <button className="flight-toggle" onClick={toggle} aria-expanded={open} aria-controls={panelId}><span><Plane /><b>Flight times & fares</b><small>Check bookable offers for your dates</small></span><ChevronDown /></button>
    {open && <div className="flight-panel" id={panelId}>
      ...
      {!loading && data && <><header><div><Source data={data} /><p>Checked {time(data.checkedAt)}</p></div><button className="refresh-rates" onClick={load}><RefreshCw /> Refresh fares</button></header>
