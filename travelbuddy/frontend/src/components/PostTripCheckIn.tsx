...
export function PostTripCheckIn({ state, onSubmit }: { state?: PostTripState; onSubmit: (rating: 1 | 2 | 3 | 4 | 5) => Promise<void> }) {
  ...
  if (state.rating) return <section className="post-trip-checkin post-trip-checkin--complete" aria-live="polite">
    ...
    <div><span className="eyebrow">Post-trip check-in</span><h2>Your profile learned from this trip.</h2><p>You rated it {state.rating} out of 5{state.adjustments?.length ? ` · ${state.adjustments.length} preference ${state.adjustments.length === 1 ? 'weight' : 'weights'} gently tuned` : ''}.</p></div>
  </section>
