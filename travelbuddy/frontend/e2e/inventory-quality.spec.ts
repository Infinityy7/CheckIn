...
async function mockWorkspace(page: Page) {
  ...
  await page.route('**/api/auth/me', (route) => json(route, { email: 'quality@example.com', intake_complete: true, cotravellers: [] }))
...
test('controlled hotel rates move into the cart without claiming a reservation', async ({ page }) => {
  ...
  await expect(page.getByText(/Test rates · Controlled provider fixture/i)).toBeVisible()
  ...
  await expect(page.getByText(/Saved cart · not reserved/i)).toBeVisible()
  await page.evaluate(() => {
    window.scrollTo(0, 0)
...
test('door-to-door transport is complete and dependency-aware', async ({ page }) => {
  ...
  await expect(page.getByText(/Test flight offers · Controlled flight fixture/i)).toBeVisible()
